...
func (cm *CertificateManager) GetCertificate(clientHello *tls.ClientHelloInfo) (*tls.Certificate, error) {
	log.Println("GetCertificate called...")
	cm.RLock()
	defer cm.RUnlock()
	return cm.certificate, nil
}

func (cm *CertificateManager) GetClientCertificate(req *tls.CertificateRequestInfo) (*tls.Certificate, error) {
	log.Println("GetClientCertificate called...")
	cm.RLock()
	defer cm.RUnlock()
	return cm.certificate, nil
}
...
func (cm *CertificateManager) setCertificate() error {
	...
	cm.Lock()
	cm.certificate = &c
	log.Println("cert: ", cm.certificate)
	cm.Unlock()
	return nil
}
